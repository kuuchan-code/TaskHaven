...
(() => {
  ...
  // ../.nvm/versions/node/v22.14.0/lib/node_modules/wrangler/templates/middleware/loader-sw.ts
  ...
  // .wrangler/tmp/bundle-olLNRw/middleware-insertion-facade.js
  ...
  __name(WSWebSocketDummy, "WSWebSocketDummy");
  ...
  async function scheduled(event, env, ctx) {
    ...
    const { data: tasks, error } = await supabase.from("tasks").select("*").gte("priority", 2).eq("completed", false);
    if (error) {
  ...
  __name(base64UrlEncode, "base64UrlEncode");
  async function sendFCMNotification(task, firebaseAccessToken) {
